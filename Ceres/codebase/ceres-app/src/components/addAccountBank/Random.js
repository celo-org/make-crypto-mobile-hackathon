...
import { setPixWalletRandomkey } from '../../store/actions/withdraw';
import * as NavigationService from '../../navigation/NavigationService'; 

//Regras de validação
const validationSchema = Yup.object().shape({
  key: Yup.string().matches(/^(?=.{32}$)[a-zA-Z0-9]*\W[a-zA-Z0-9]*$/, 'Formato incorreto').required('Este campo é obrigatório'),
})
  ...
  const saveKey = (variant, key) => { 
    ...
    if(keyValues[variant].initialKeyValue.key == null){  
      sendPixKey(variant, key).then(response=>{
        ...
        dispatch(setPixWalletRandomkey( response.data.id, response.data.key));
          ...
          <Card disabled={true}>
            <View style = {{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
              <View style = {{margin: 8, height: 50, width: 50, borderRadius: 25, backgroundColor: '#3366FF', justifyContent: 'center', alignItems: 'center'}}>
                <Icon fill='white' style = {{height: 24, width: 24}} name='hash-outline'/>
              </View> 
