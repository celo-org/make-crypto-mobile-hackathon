...
import { setPixWalletCPFCNPJ } from '../../store/actions/withdraw';
import * as NavigationService from '../../navigation/NavigationService'; 

//Regras de validação
const validationSchema = Yup.object().shape({
  key:  Yup.string().matches(/^\d{3}\.\d{3}\.\d{3}\-\d{2}$/, 'Formato incorreto').required('Este campo é obrigatório'),
})

export const CPFComponent = (props) => {
  ...
  const keyValues = {
    ...
    'CPF': {title: 'CPF', initialKeyValue: wallet.fiatWallet.cpf_cnpj}, 
  ...
  const saveKey = (variant, key) => { 
    ...
    if(keyValues[variant].initialKeyValue.key == null){  
      sendPixKey(variant, key).then(response=>{
        ...
        dispatch(setPixWalletCPFCNPJ( response.data.id, response.data.key));
  ...
  return (
    <Formik
      ...
      let Key_noMask = Key_ref.getRawValue();
      
      saveKey(variant, Key_noMask) 
      resetForm({key: Key_noMask})
      setSubmitting(false);
    }}
          ...
          <Card disabled={true}>
            <View style = {{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
              <View style = {{margin: 8, height: 50, width: 50, borderRadius: 25, backgroundColor: '#3366FF', justifyContent: 'center', alignItems: 'center'}}>
                <Icon fill='white' style = {{height: 24, width: 24}} name='person-outline'/>
              </View> 
