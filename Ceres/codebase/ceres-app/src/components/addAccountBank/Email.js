...
import { setPixWalletPhone, setPixWalletEmail } from '../../store/actions/withdraw';
import * as NavigationService from '../../navigation/NavigationService'; 

//Regras de validação
const validationSchema = Yup.object().shape({
  key: Yup.string().label('Email').email('Insira um email válido').required('Por favor insira um email'),
})
...
export const EmailComponent = (props) => {
  ...
  const saveKey = (variant, key) => { 
    ...
    if(keyValues[variant].initialKeyValue.key == null){  
      sendPixKey(variant, key).then(response=>{
        ...
        dispatch(setPixWalletEmail( response.data.id, response.data.key));
  ...
  return (
    <Formik
      ...
      saveKey(variant, values.key) 
      resetForm({key: values.key})
      setSubmitting(false);
    }}
          ...
          <Card disabled={true}>
            <View style = {{flex: 1, justifyContent: 'center', alignItems: 'center'}}>
              <View style = {{margin: 8, height: 50, width: 50, borderRadius: 25, backgroundColor: '#3366FF', justifyContent: 'center', alignItems: 'center'}}>
                <Icon fill='white' style = {{height: 24, width: 24}} name='email-outline'/>
              </View> 
