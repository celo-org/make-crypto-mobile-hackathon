...
export class Survey extends Component {
  static propTypes = {
    ...
    renderEmailInput: PropTypes.func,
    renderPhoneInput: PropTypes.func,
    renderDateInput: PropTypes.func,
    renderNumberInput: PropTypes.func,
    renderRatingInput: PropTypes.func,
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      answers: [],
      titleOfTask: props.titleOfTask,
      points: props.points,
      reward: props.reward,
      questionsNo: props.questionsNo,
    };
  ...
  validateMultipleSelectionSurveyAnswers() {
    ...
    if (
      !this.props.survey[currentQuestionIndex].questionType ===
    ...
    if (
      this.props.survey[currentQuestionIndex].questionSettings.maxMultiSelect
    ) {
    ...
    if (
      this.props.survey[currentQuestionIndex].questionSettings.minMultiSelect
    ) {
    ...
    if (
      ...
      answers[currentQuestionIndex].content.length >= minMultiSelect
    ) {
      return true;
    } else {
      return false;
  ...
  validateVideo() {
    ...
    if (!this.props.survey[currentQuestionIndex].questionType === 'Video') {
      throw new Error('validateVideo was asked to validate a non Video item');
    }
    let minWatchTime = 10;
    minWatchTime = min_time_view;
    if (
      answers[currentQuestionIndex] &&
      answers[currentQuestionIndex].content >= minWatchTime
    ) {
      return true;
    } else {
      return false;
  ...
  validateEmail() {
    ...
    if (!this.props.survey[currentQuestionIndex].questionType === 'Email') {
      throw new Error('ValidateEmail was asked to validate a non Video item');
    }
    ...
    if (
      answers[currentQuestionIndex] &&
      reg.test(answers[currentQuestionIndex].content)
    ) {
      return true;
    } else {
      return false;
  ...
  validatePhone() {
    ...
    if (!this.props.survey[currentQuestionIndex].questionType === 'Phone') {
      throw new Error('ValidatePhone was asked to validate a non Video item');
    }
  ...
  validateDate() {
    ...
    if (!this.props.survey[currentQuestionIndex].questionType === 'Date') {
      throw new Error('ValidateDate was asked to validate a non Video item');
    }

    let reg = /^([0]?[1-9]|[1|2][0-9]|[3][0|1])[./-]([0]?[1-9]|[1][0-2])[./-]([0-9]{4}|[0-9]{2})$/;
  ...
  renderFinishOrNextButton() {
    ...
    switch (survey[currentQuestionIndex].questionType) {
      ...
      case 'Info':
        ...
        break;
      case 'Video':
        enabled = this.validateVideo();
        break;
      case 'Email':
        enabled = this.validateEmail();
        break;
      case 'Phone':
        enabled = this.validatePhone();
        break;
      case 'Date':
        enabled = this.validateDate();
        break;
      default:
        enabled =
          Boolean(answers[currentQuestionIndex]) &&
          (answers[currentQuestionIndex].content ||
            answers[currentQuestionIndex].content === 0);
        break;
    }
    ...
    return this.props.renderNext(() => {
      ...
      if (this.props.survey[currentQuestionIndex + 1].has_prerequisite) {
        ...
        for (let j = 0; j < survey_data.length; j++) {
          if (
            this.checkTaskAnswerBy(
              this.findTaskById(survey_data[j].prerequisite_question),
              survey_data[j].condition,
            )
          ) {
            popNext = false;
          }
  ...
  validateSelectionGroupSettings(questionSettings, currentQuestionIndex) {
    ...
    const {
      allowDeselect,
      defaultSelection,
      autoAdvance: autoAdvanceThisQuestion,
    } = questionSettings;
    ...
    if (
      ...
      (this.props.autoAdvance || autoAdvanceThisQuestion)
    ) {
      throw new Error(
    ...
    if (
      autoAdvanceThisQuestion !== undefined &&
  ...
  renderSelectionGroup() {
    ...
    this.validateSelectionGroupSettings(
      this.props.survey[currentQuestionIndex].questionSettings,
      currentQuestionIndex,
    );
    if (!this.selectionHandlers[currentQuestionIndex]) {
      if (!this.props.survey[currentQuestionIndex].questionSettings) {
        ...
        const {allowDeselect, defaultSelection} = this.props.survey[
          currentQuestionIndex
        ].questionSettings;
          ...
          setTimeout(
            () =>
              this.updateAnswer({
                ...
                model: 'QUESTION',
                content:
                  survey[currentQuestionIndex].options[options.defaultSelection]
                    .optionText,
      ...
      <View style={containerStyle}>
        ...
        <ScrollView style={mainSurveyContainer}>
          {this.props.renderQuestionText
            ? this.props.renderQuestionText(
                this.props.survey[currentQuestionIndex].questionText,
              )
            : null}
          <SelectionGroup
            onPress={
              this.selectionHandlers[currentQuestionIndex].selectionHandler
            }
        ...
        <ScrollView style={mainSurveyContainer}>
          ...
          <SelectionGroup
            ...
            getAllSelectedItemIndexes={
              this.selectionHandlers[currentQuestionIndex]
                .getAllSelectedItemIndexes
            }
            renderContent={renderSelector}
            containerStyle={selectionGroupContainerStyle}
            onItemSelected={(item, allSelectedItems) => {
              ...
              for (var i = 0; i < allSelectedItems.length; i++) {
                compiledOptions.length > 0
                  ? (compiledOptions += '@#' + allSelectedItems[i].optionText)
                  : (compiledOptions += allSelectedItems[i].optionText);
              }
              ...
              (autoAdvanceThisQuestion || this.props.autoAdvance) &&
                this.autoAdvance();
            }}
  ...
  renderNumeric() {
    ...
    return (
      <View style={containerStyle}>
        ...
        <ScrollView style={mainSurveyContainer}>
          {this.props.renderQuestionText
            ? this.props.renderQuestionText(questionText)
            : null}
          {renderNumericInput(
            (content) => {
  ...
  render() {
    ...
    switch (survey[currentQuestion].questionType) {
      ...
      case 'Email':
        return this.renderEmailInputElement();
      case 'Phone':
        return this.renderPhoneInputElement();
      case 'Date':
        return this.renderDateInputElement();
      case 'Number':
        return this.renderNumberInputElement();
      case 'Rating':
        return this.renderRatingInputElement();
      case 'TextInput':
